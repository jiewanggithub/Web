 ...
 */
let pavement_texture = new T.TextureLoader().load("../for_students/images/pavement.jpg");
let pavement = new T.TextureLoader().load("../for_students/images/emptypavement.jpg");
let stop = new T.TextureLoader().load("../for_students/images/stopsign.jpg");
export class CircularTrack extends GrObject {
  constructor(params = {}) {
    let radius = params.radius || 14;
...
export class LinearTrack extends GrObject {
  constructor(x,y,z) {
    let length = x;
    let width = y;
    let ring = new T.BoxGeometry(y,0.1,x);
    let material = new T.MeshStandardMaterial({
      map: pavement_texture,
      roughness: 1.0,
    });
    let mesh = new T.Mesh(ring, [
      material,
      material,
      material,
      material,
    ...
    let group = new T.Group();
    group.add(mesh);
    super(`LinearTrack`, group)
  }
}
export class EmptyTrack extends GrObject {
  constructor(x,y) {
    let length = x;
    let width = y;
    let ring = new T.BoxGeometry(y,0.1,x);
    ...
    let group = new T.Group();
    group.add(mesh);
    super(`EmptyTrack`, group)
  }
}
export class stopsign extends GrObject {
  constructor() {
    let sign = new T.BoxGeometry(0.1,5,5);
    let pole = new T.BoxGeometry(0.3,7,0.1);
    let silver_mat = new T.MeshStandardMaterial({color:"silver"});
    let material = new T.MeshStandardMaterial({
      map: stop,
      roughness: 1.0,
    });
    let mesh = new T.Mesh(sign, [
      material,
      material,
      material,
      material,
    ...
    let mesh2 = new T.Mesh(pole,silver_mat);
    mesh2.position.set(0,2,0);
    let group = new T.Group();
    group.add(mesh);
    group.add(mesh2);
    super(`Stopsign`, group)
  }
}
/**
